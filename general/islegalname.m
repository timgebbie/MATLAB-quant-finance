...
%   $Revision: 1.1.6.3 $   $Date: 2007/08/15 17:18:55 $
...
switch class(varargin{1}),
   case 'char',
      if ischar(varargin{1}) & size(varargin{1}, 1) ~= 1,
         error('finance:islegalname:invalidCharSize', ...
            'A single string is expected. Use a cell array for a list of strings.');
      end
      varname = varargin{1};
   ...
   case 'cell',
      if size(varargin{1}, 1) ~= 1 & size(varargin{1}, 2) ~= 1,
         error('finance:islegalname:invalidCellDim', ...
            'A column or row cell array is expected.');
      end
   ...
   otherwise,
      error('finance:islegalname:invalidInputType', 'Invalid input argument type.');
end
