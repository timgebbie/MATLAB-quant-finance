...
%         for i=1:mp % for debugging [CHECK matrix logic]
        ...
        %     for debugging : 
            ...
            E_t_dP_NegComplexExpDiffPrice = gpuArray(zeros(1,size(K,2)));
            E_t_dP_PosComplexExpDiffPrice = gpuArray(zeros(1,size(K,2)));
            % compute on GPU
            ...
            E_t_dP_NegComplexExpDiffPrice = DiffP' * exp((complex(0,-T(2:end,1)*K(1,:))));
            E_t_dP_PosComplexExpDiffPrice = DiffP' * exp((complex(0,T(2:end,1)*K(1,:))));
            % --------------------------------------------------
            % <-- off GPU ---|
            % data from GPU
            e_t_dp(i,1).PosComplexExpDiffPrice = gather(E_t_dP_PosComplexExpDiffPrice);
            e_t_dp(i,1).NegComplexExpDiffPrice = gather(E_t_dP_NegComplexExpDiffPrice);
        end
