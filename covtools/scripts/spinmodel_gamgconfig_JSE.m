...
for ti=t0:tend,
    ...
    for j=1:max(cx(ti,:))
        ...
        cs(ti,j) = sum(sum(r0(sindex,sindex)));
        % #3 : compute gs the coupling strengths
        gs(ti,j) = sqrt(max(0,(cs(ti,j)-ns(ti,j)) ./ (ns(ti,j) .* ns(ti,j) - ns(ti,j)))); % Eq.4, Marsili (2002)
    end        
end

%% Save cluster memb ership data
save workspace_gamgconfig_JSE_D_cx;
...
cs_jse = cs;
gs_jse = gs;
save workspace_gamgconfig_JSE_D cx_jse ns_jse cs_jse gs_jse;
