%% Script file to generate cluster and display ICB histograms (WEEKLY with market)
%
...
% c = datastream('DS:XQTM001','VISOR527','Datastream','http://dataworks.thomson.com/Dataworks/Enterprise/1.0');
% data = fetch(c, sprintf('LJSEOVER~LIST~=MNEM,MV,ICBSUC,ICBIN,ICBIC~AA~@%s',datestr(today,29)));

%% initialise
load('data/JSE_D_LJSEOVER_31Jan1994_22Jan2009');
load('data/JSE_LJSEOVER_ICB');
...
clear Dmov;
aviobj = avifile('JSE_W_LJSEOVER_SPIN_MARKET.avi','FPS',1);
aviobj1 = avifile('JSE_W_LJSEOVER_SPIN_MARKET_HIST.avi','FPS',1);
scrsz = get(0,'ScreenSize');
cxfts = fints;
% axes1 = axes(fig,'Position',[0.13 0.688 0.775 0.237]);
t0 = 60;
for ti=t0:7:size(s,3),
    c0 = squeeze(s(:,:,ti));
    ...
    for cj=1:size(cixy,1)
        ...
        % histogram data
        thindex=ismember(tick2tick(data.MNEM),tickersc{cj});
        icb{cj}=str2double(data.ICBIC(thindex));
    end
    % plot
    % clf('reset');
    ...
    pn = ceil(sqrt(length(t)+1));
    pm = ceil((length(t)+1)/pn);
    for i=1:cj+1
        h = subplot(pm,pn,i);
        if i>1
            [n,x]=hist(icb{i-1},0:1000:10000);
            barh(x,n);
            set(gca,'YLim',[-500,9500]);
        else
            % figure;
            axes(h);
            set(gca,'YLim',[-500,9500],'YTick',[0:1000:9000],'XLim',[0,10]);
            [icb0,indx2]=unique(str2double(data.ICBIC));
            icbn0 = data.ICBIN(indx2);
            for j=1:9
                text(1,icb0(j),icbn0(j),'FontSize',10);
            end
        end
    end
    F1 = getframe(fig1);
